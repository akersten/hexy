/* 
 ...
 Within this file are all the functions for Hexy. Call initEverything with the size of the board you want to play to get
 started.

 Changelog:

 1.1 Corners are now 'universal', so a path from either side to an opposite corner constitutes a victory for either
 player. (Corners are no longer specific to one player.)
...
// How many hexagons tall is the center pillar of this board? Also the length of any one side of the board.
var gameSize = 8;

// How many pixels wide & tall is one hexagon picture?
var tileImageSize = 48;

// How many pixels are we shifting horizontally when we tessalate these hexagons? There's probably a better way to
// calculate this but I did it in Photoshop by just nudging the tile over until it tesselated and counting the pixels.
var tileHorizontalSeparation = 36;

// They're used in the calculation of tile position since we index them like we were indexing a square.
var tileVerticalSeparation = 24;

// Different hex PNG images we'll be using to represent the board.
var imgBorderNormal, imgBorderHighlight, imgBorderGoalA, imgBorderGoalB, imgFillNormal, imgFillHighlight, imgFillA,
    imgFillB, imgBorderCorner;
...
/**
 * The game data will be stored in a square array with [0][0] representing the leftmost hexagon on the board, The first
 * coordinate specifies the column and the second the row. Imagine a square tilted 45 degrees counterclockwise, and
 * that is the coordinate representation.
 * 
 * Technically, this isn't really a 2-d array, more like an array of arrays, but in practice it will work the same way.
 * It's created in the reset() function.
 * 
 * The way this data structure is going to work is, it'll be an array2 of 'int's following the following rules:
 * 0 = Normal tiles (unoccupied)
 * 1 = Occupied by A
 * 2 = Occupied by B
 * 3 = Unoccupied highlighted tile
 * 
 * Whether a tile is a goal tile or not (aka, what border to draw) will be determined by the repaint method.
 */
var tiles;

/**
 * Creates the canvas context and assigns it to the 'global' variable context. Call once only once per page load please.
 *
...
function paintBackground() {
    ...
    // Well, if the game's over, paint the BG the color of the victor. Victor is whoever's turn it still is since the
    // turn switch doesn't happen until after victory conditions are checked.
    if (gameOver) {
        if (!turn) {
            // Player A won
 ...
 */
function repaint() {
    // Clear the background first, don't want any remnants left in places we forgot to repaint.
    paintBackground();

    // Draw the tiles and tile borders. Most of this is positioning code for the tiles.
    ...
    for (var i = 0; i < gameSize; i++) {
        for (var j = 0; j < gameSize; j++) {
            ...
            // As far as the corners go, we'll just paint everything normally pretending they don't exist, until we get
            // to the last lines which will do the additional check. Cleaner code that way.
            ...
            // Check if it's a corner piece and quickly swap the image pointer to represent that before we paint it.
            ...
            // Figure out where to paint this tile. Should be fairly easy, Use the calculated center of the board and go
            // from there. The middle tile needs to go in the center, so we'll normalize around that.
...
function resolveCanvasPositionToArrayIndex(x, y) {
    // We'll kind of reverse engineer the position algorithms to determine which tile index *should* be here, and if
    // it's out of range then reject it.

    // I treated the position-determining algorithm from the repaint method as the x and y position given i and j, so
    // we basically find the inverse of that by solving the two systems for i and j. It's not pretty, but it should
    // work.
...
/**
 * Called by the onmousemove attribute of the canvas - keeps the local mouse position up to date, and then repaints the
 * screen to reflect any changes.
 */
function updateMousePosition(event) {
    ...
    // If the current tile is unowned, highlighht it to indicate we could move there.
    if (tileIdx != -1) {
...
/**
 * The mouse was clicked in our canvas - check to see if mouseX and mouseY are a tile, and then update that tile for the
 * player. After that, check for victory conditions. And then repaint.
 */
function mouseClicked() {
    ...
    var tileIdx = resolveCanvasPositionToArrayIndex(mouseX, mouseY);
    if (tileIdx != -1) {
        // The type will either be 3 (likely, since we're hovering over it) or 1 on the off-chance we've gone somewhere
        // without hovering first (but could happen if we're on a mobile device or something).
...
/**
 * Checks to see if there exists a path between this tile through tiles colored similarily to it, to A's high side.
 */
function hasPathToHighGround(tileIdx) {
    ...
    // Base case - we either wind up in the high side, or we have no other connections to follow except the one we came
    // from.
    if (!turn) {
